...
class NetCombatPage extends StatelessWidget {
  ...
  NetCombatPage({super.key, required this.roomInfo, required this.userName}) {
      ...
      builder: (__, step, _) {
        return Column(
          children: [
            ...
            ...(step.index >= TurnGameStep.playerTrun.index
                ? _buildCombat()
                : _buildPrepare(step)),

            Expanded(
              child: MessageList(networkEngine: _combatManager.networkEngine),
            ),
...
class BattleButtonRegion extends StatelessWidget {
  final NetCombatManager combatManager;
